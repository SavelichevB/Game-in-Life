...
namespace GameHeith
{
    public partial class Form1 : Form
    {
        ...
        void SetFormSize()
        {
            ...
            this.Height = (mapSize + 1) * mapSize + 45;
        }
        ...
        void BuildMenu()
        {
            ...
            var restart = new ToolStripMenuItem("Рестарт");
        ...
        private void UpdateStates(object sender, EventArgs e)
        {
           CalculateNextState();
            ...
            if (CheckGenerationDiad())
            {
                mainTimer.Stop();
                MessageBox.Show("Поколение отжило!");

            }
        }

        bool CheckGenerationDiad()
        {
        ...
        void CalculateNextState()
        {
            for (int i = 0; i < mapSize; i++)
            {
                for (int j = 0; j < mapSize; j++)
                {
                    var countNeighboors = CountNeigboors(i, j);
        ...
        void DisplayMap()
        {
            for (int i = 0; i < mapSize; i++)
            {
                ...
                {
                    if (currentState[i, j] == 1)
                        cells[i,j].BackColor = Color.Green; //color
                    else cells[i,j].BackColor = Color.White;
                }
            }
        }

        int CountNeigboors(int i, int j)
        {
            var count = 0;
        ...
        private void OnCellClick(object sender, EventArgs e)
        {
            ...
            if (!isPlaying)
            {
             ...
             var j = pressedButton.Location.X / cellSize ;
                ...
                {
                    currentState[i,j] = 1;
                    cells[i, j].BackColor = Color.Green; //colors
                }
                else
                {
                    currentState[i, j] = 0;
